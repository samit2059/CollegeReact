...
// in filter we will get all the elemnent s with the true condition:
let result = myArr.filter((element, index) =>{
    return element>9;
})
console.log(result);
// in find we will get only the one value in index with value being true.
let finder = myArr.find((element, let index) =>{
    return element>30;
})
console.log(finder);